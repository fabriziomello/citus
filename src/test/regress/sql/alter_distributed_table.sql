...
SELECT table_name, citus_table_type, distribution_column, shard_count FROM pg_catalog.citus_tables
    WHERE table_name IN ('dist_table', 'colocation_table', 'colocation_table_2');
SELECT STRING_AGG(table_name::text, ', ' ORDER BY 1) AS "Colocation Groups" FROM pg_catalog.citus_tables
...
SELECT table_name::text, distribution_column, shard_count FROM pg_catalog.citus_tables WHERE table_name::text LIKE 'partitioned\_table%' ORDER BY 1;
...
SELECT table_name::text, shard_count, access_method FROM pg_catalog.citus_tables WHERE table_name::text = 'columnar_table';
...
SELECT table_name, shard_count FROM pg_catalog.citus_tables WHERE table_name::text = 'metadata_sync_table';
