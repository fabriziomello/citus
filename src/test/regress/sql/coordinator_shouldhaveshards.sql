...
SELECT table_name, citus_table_type, distribution_column, shard_count FROM pg_catalog.citus_tables WHERE table_name::text LIKE 'adt%';
SELECT STRING_AGG(table_name::text, ', ' ORDER BY 1) AS "Colocation Groups" FROM pg_catalog.citus_tables WHERE table_name::text LIKE 'adt%' GROUP BY colocation_id ORDER BY 1;
...
SELECT table_name, citus_table_type, distribution_column, shard_count FROM pg_catalog.citus_tables WHERE table_name::text = 'adt_table';
SET client_min_messages TO DEFAULT;
